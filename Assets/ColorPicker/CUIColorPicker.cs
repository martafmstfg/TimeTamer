...
public class CUIColorPicker : MonoBehaviour
{
    ...
    private Action<Color> cambiarColor = new Action<Color>(CambiarColor);

    private Color[] coloresHue = new Color[3];

    [SerializeField]
    private bool togglePiel, toggleOjo, togglePelo;

    private static CUIColorPicker cuicp;

    private static AvatarEditor avatarEditor;
    private static AvatarFileManager avatarFile;

    [Inject]
    public void Init(AvatarEditor ae, AvatarFileManager af) {
        avatarEditor = ae;
        avatarFile = af;
    }    
    ...
    private static void CambiarColor(Color color) {

        if(cuicp.togglePiel) {
            avatarEditor.CambiarColor(0, color);
            cuicp.coloresHue[0] = color;
        }
        else if (cuicp.toggleOjo) {
            avatarEditor.CambiarColor(1, color);
            cuicp.coloresHue[1] = color;
        }
        else if (cuicp.togglePelo) {
            avatarEditor.CambiarColor(2, color);
            cuicp.coloresHue[2] = color;
        }
    }

    public void TogglePiel (bool b) {
        togglePiel = b;

        if (b) CambiarHue(0);
    }
    public void ToggleOjo(bool b) {
        toggleOjo = b;

        if(b) CambiarHue(1);
        
    }
    public void TogglePelo(bool b) {
        togglePelo = b;

        if (b) CambiarHue(2);
    }

    private void GuardarHueInicial () {
        //Guardar colores iniciales
        string colorInicial = avatarFile.GetColor(0);
        ColorUtility.TryParseHtmlString(colorInicial, out Color aux);
        coloresHue[0] = aux;

        colorInicial = avatarFile.GetColor(1);
        ColorUtility.TryParseHtmlString(colorInicial, out aux);
        coloresHue[1] = aux;

        colorInicial = avatarFile.GetColor(2);
        ColorUtility.TryParseHtmlString(colorInicial, out aux);
        coloresHue[2] = aux;
    }

    private void CambiarHue (int i) {        
        Color = coloresHue[i];
    }
}
